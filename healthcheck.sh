...
API=$(timeout 10 curl -s -o /dev/null -w "%{http_code}" https://pharma-api-brax.onrender.com/api/v1/batches)
echo "API: ${API:-TIMEOUT}"
BATCOUNT=$(timeout 5 curl -s https://pharma-api-brax.onrender.com/api/v1/batches | grep -c '"lot_number"')
